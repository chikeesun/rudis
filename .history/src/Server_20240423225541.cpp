 ...
 * @LastEditTime: 2024-04-23 22:55:01
...
void handle_client(int client_fd) {
  ...
  while (true) {
    ...
    if (command.find("*") == std::string::npos) continue;
    int command_count = std::stoi(command.substr(
        command.find("*") + 1, command.find("\r\n") - command.find("*") - 1));
    for (int i = 0; i < command_count; i++) {
      int method_len = std::stoi(command.substr(
          command.find("$") + 1, command.find("\r\n") - command.find("$") - 1));
    }
