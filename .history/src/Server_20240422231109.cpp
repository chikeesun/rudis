 ...
 * @LastEditTime: 2024-04-22 23:09:42
...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    received_data = received_data.substr(l, r - l + 1);
    r = r - l;
    l = 0;
    while (l < r) {
      std::string command;
      int sep = received_data.find_first_of("\n");
      if (sep != std::string::npos) {
        command = received_data.substr(l + 1, sep - l - 1);
        l = sep;
      } else {
        command = received_data.substr(l + 1, r - l - 1);
        l = r;
      }
